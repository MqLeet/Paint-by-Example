    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                test_model_kwargs={}
                test_model_kwargs['inpaint_mask']=Resize([opt.H, opt.W])(mask_tensor.to(device))
                test_model_kwargs['inpaint_image']=Resize([opt.H, opt.W])(inpaint_image.to(device))
                print('0',mask_tensor.to(device).size())
                print('1',inpaint_image.to(device).size())
                ...
                if not opt.skip_save:
                    for i,x_sample in enumerate(x_checked_image_torch):
                        ...
                        all_img=[]
                        all_img.append(un_norm(Resize([opt.H, opt.W])(image_tensor[i]).cpu()))
                        all_img.append(un_norm(Resize([opt.H, opt.W])(inpaint_image[i]).cpu()))
                        ref_img=ref_tensor
